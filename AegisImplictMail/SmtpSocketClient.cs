...
namespace AegisImplicitMail
{
    ...
    public string Password
    {
      ...
      set {_password = value;}
        ...
        public const string mailAddressSeparator = ",";
        ...
        public void SendMail(AbstractMailMessage message)
        {
            ...
            lock (this)
            {
                ...
                if (EsablishSmtp())
                {
                    ...
                    {
                        buf.Append(mailAddressSeparator);
        ...
        private void SendMessageBody(StringBuilder buf)
        {
            ...
            var sendAsHtml = _sendAsHtml || MailMessage.IsBodyHtml;

            buf.Length = 0;
            //declare mime info for message
            _con.SendCommand("MIME-Version: 1.0");
            if (!sendAsHtml ||
                (sendAsHtml && ((MimeAttachment.InlineCount > 0) || (MimeAttachment.AttachCount > 0))))
            {
                _con.SendCommand("Content-Type: multipart/mixed; boundary=\"#SEPARATOR1#\"\r\n");
                _con.SendCommand("This is a multi-part message.\r\n\r\n--#SEPARATOR1#");
            }
            if (sendAsHtml)
            {
                _con.SendCommand("Content-Type: multipart/related; boundary=\"#SEPARATOR2#\"");
                _con.SendCommand(encodingQuotedPrintable);
                _con.SendCommand("--#SEPARATOR2#");
            }
            if (sendAsHtml && MimeAttachment.InlineCount > 0)
            {
                _con.SendCommand("Content-Type: multipart/alternative; boundary=\"#SEPARATOR3#\"");
                _con.SendCommand(encodingQuotedPrintable);
                _con.SendCommand("--#SEPARATOR3#");
          ...
          //      _con.SendCommand("Content-Type: text/html; charset=iso-8859-1");
            ...
            {
                if (sendAsHtml || MailMessage.IsBodyHtml)
                {
                    _con.SendCommand(encodingHtmlHeader);
                    _con.SendCommand(encodingQuotedPrintable);
		...
		private void SendAttachments(StringBuilder buf, AttachmentLocation type)
		{
		    ...
		    string separator = type == AttachmentLocation.Attachmed ? "--#SEPARATOR1#" : "--#SEPARATOR2#";
			buf.Length = 0;
			...
			{									
                ...
                var s = !string.IsNullOrEmpty(attachment.FileName) ?
				...
				//_con.SendCommand(separator);
				buf.Append("Content-Type: ");
				buf.Append(attachment.ContentType);
				buf.Append("; name=");
				buf.AppendLine(Path.GetFileName(attachment.FileName ?? attachment.Name));
				
				buf.AppendLine("Content-Transfer-Encoding: base64");
				//buf.Length = 0;
				...
				buf.AppendLine(Path.GetFileNameWithoutExtension(attachment.FileName ?? attachment.Name));				
                ...
                s.CopyTo(ms);
                _con.SendCommand(TransferEncoder.ToBase64(ms.ToArray()));
				//s.Close();
                buf.AppendLine();
				_con.SendCommand(buf.ToString());
                buf.Length = 0;
