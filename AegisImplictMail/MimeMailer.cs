...
namespace AegisImplicitMail
{
	...
	public interface IMimeMailer
	{
		NetworkCredential Credentials { get; set; }
		
		bool EnableImplicitSsl { get; set; }
		...
		/// </summary>
		AuthenticationType AuthenticationMode { get; set; }
		...
		MimeMailMessage MailMessage { get; set; }
		SslMode SslType { get; set; }
		bool DsnEnabled { get; }
		...
		AbstractMailMessage GenerateMail(IMailAddress sender, List<IMailAddress> toAddresses, List<IMailAddress> ccAddresses, List<IMailAddress> bccAddresses, List<string> attachmentsList,
		...
		void Send(AbstractMailMessage message, SendCompletedEventHandler onSendCallBack = null);

		void Send(string from, string recipiant, string subject, string body);
		...
		/// Send the message.
		/// </summary>
		void SendMail(AbstractMailMessage message);

		/// <summary>
		/// Send the message on a seperate thread.
		/// </summary>
		void SendMailAsync(AbstractMailMessage message = null);
    ...
    /// </summary>
    public class MimeMailer :SmtpSocketClient, IMailer, IMimeMailer
	{
