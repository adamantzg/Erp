(function () {
    ...
    returnFeedbacks.$inject = ['$scope','$timeout','$state','$uibModal' ,'factoryClaims','$stateParams','version', 'types'];
    
    function returnFeedbacks($scope, $timeout,$state,$uibModal, factoryClaims,$stateParams,version, types) {

        $scope.types = types;
        $scope.ctrlName = 'returnFeedbacksCtrl.js';
        ...
        $scope.PARAMS = $stateParams.dealerId;
        ...
        $scope.GetProducts = function (term) {
            $scope.busy = true;
            $scope.Products = [];
            factoryClaims.getProducts(term,$scope.optAmaraOrCross).then(
                function (data) {
                    $scope.Products = data;
        ...
        $scope.open = function () {
            $uibModal.open({
                templateUrl: $scope.error ? "/AngularApps/DealerUSA/claims/returnError.html?v="+version.v : "/AngularApps/DealerUSA/claims/returnSaveSuccessMessage.html?v="+version.v,
                    ...
                    function (dealers) {
                        ...
                        if ($scope.PARAMS !== "")
                        {
                            for (var i = 0; i < dealers.length; i++) {
                                ...
                                if(dealers[i].customer==$scope.PARAMS){
                                    $scope.dealer = dealers[i];
                                    $scope.model.dealer_id = dealers[i].customer;
                                }
                            }
        ...
        function removeImage(name) {
            factoryClaims.getReference(name).then(
               function (data) {
                   $scope.model.return_no = data;
               })
        }
