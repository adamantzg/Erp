...
namespace erp.Model.DAL
{
    public class After_sales_enquiry_sitevisit_statusDAL
	{
		...
		public static List<After_sales_enquiry_sitevisit_status> GetAll()
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM after_sales_enquiry_sitevisit_status", conn);
		...
		public static After_sales_enquiry_sitevisit_status GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM after_sales_enquiry_sitevisit_status WHERE status_id = @id", conn);
		...
		public static void Create(After_sales_enquiry_sitevisit_status o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                MySqlCommand cmd = Utils.GetCommand("SELECT MAX(status_id)+1 FROM after_sales_enquiry_sitevisit_status", conn);
		...
		public static void Delete(int status_id)
		{
            ...
            {
                conn.Open();
				MySqlCommand cmd = Utils.GetCommand("DELETE FROM after_sales_enquiry_sitevisit_status WHERE status_id = @id" , conn);
