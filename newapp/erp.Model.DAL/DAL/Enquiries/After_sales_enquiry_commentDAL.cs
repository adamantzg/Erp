...
namespace erp.Model.DAL
{
    public class After_sales_enquiry_commentDAL
	{
        ...
        public static List<After_sales_enquiry_comment> GetForEnquiry(int enquiry_id, CompanyType role, bool includeFiles = true)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(@"SELECT after_sales_enquiry_comment.*,Creator.userwelcome AS creator, Recipient.user_name AS respondto_name, Recipient.distributor AS respondto_distributor FROM after_sales_enquiry_comment 
        ...
        public static After_sales_enquiry_comment GetLastCommentForRole(int enquiry_id, CompanyType role)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM after_sales_enquiry_comment WHERE enquiry_id = @enquiry_id AND (creator_role = @company_role OR respond_to_role = @company_role) ORDER BY datecreated DESC LIMIT 1 ", conn);
                cmd.Parameters.AddWithValue("@enquiry_id", enquiry_id);
                cmd.Parameters.AddWithValue("@company_role", role);
                MySqlDataReader dr = cmd.ExecuteReader();
                if (dr.Read())
		...
		public static After_sales_enquiry_comment GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM after_sales_enquiry_comment WHERE comment_id = @id", conn);
		...
		public static void Delete(int comment_id)
		{
            ...
            {
                conn.Open();
				MySqlCommand cmd = Utils.GetCommand("DELETE FROM after_sales_enquiry_comment WHERE comment_id = @id" , conn);
