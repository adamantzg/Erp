...
namespace erp.Model.DAL
{
    public class After_sales_enquiryDAL
	{
        ...
        public static List<After_sales_enquiry> GetAll(string sortField = "", int? status_id = null, DateTime? closedFrom = null, DateTime? closedTo = null)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM after_sales_enquiry", conn);
        ...
        public static List<After_sales_enquiry> GetForManufacturer(User user, string sortField = "", bool completedOnly = false, DateTime? closedFrom = null, DateTime? closedTo = null)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(GetSelectSqlForManufacturer() + " WHERE admin_permissions.userid = @user_id", conn);
		...
		public static After_sales_enquiry GetById(int id, bool loadChildren = true, User currentUser = null)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(GetSelectSql() + " WHERE enquiry_id = @id", conn);
        ...
        public static List<User> GetCommentThreadUsers(int enquiry_id)
        {
            ...
            {
                conn.Open();
                //MySqlCommand cmd = Utils.GetCommand(@"SELECT creator.*, company.* FROM After_sales_enquiry_comment INNER JOIN userusers AS creator ON After_sales_enquiry_comment.created_userid = creator.useruserid
                //                                    INNER JOIN users company ON userusers.user_id = users.user_id WHERE After_sales_enquiry_comment.enquiry_id = @enquiry_id", conn);
                MySqlCommand cmd = Utils.GetCommand(@"SELECT creator.* FROM After_sales_enquiry_comment INNER JOIN userusers AS creator ON After_sales_enquiry_comment.created_userid = creator.useruserid
...
//                }
		...
		public static void Delete(int enquiry_id)
		{
            ...
            try
            {
                ...
                conn.Open();
                tr = conn.BeginTransaction();
                MySqlCommand cmd = Utils.GetCommand("",conn, tr);
        ...
        public static int GetNextReferenceNumber(int distributor_id)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(@"SELECT COALESCE(COUNT(*),0)+1 FROM after_sales_enquiry INNER JOIN userusers ON after_sales_enquiry.created_userid = userusers.useruserid
