...
namespace erp.Model.DAL
{
    public class Order_headerDAL
	{
		...
		public static List<Order_header> GetAll()
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM order_header", conn);
        ...
        public static List<Order_header> GetCreatedInPeriod(DateTime? from, DateTime? to, bool gbie_only = true)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(string.Format(@"SELECT order_header.*,(SELECT MAX(po_req_etd)
        ...
        public static List<Order_header> GetProducedInPeriod(DateTime? from = null, DateTime? to = null,DateTime? created_until = null, bool gbie_only = true)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(string.Format(@"SELECT order_header.*,(SELECT MAX(po_req_etd) FROM porder_header WHERE soorderid = order_header.orderid) AS po_req_etd, users.*
        ...
        public static List<OrderMonthlyData> GetQtyByMonth(string cprod_code1, int company_id, DateTime fromMonth, DateTime toMonth)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(@"SELECT custpo,  YEAR(req_eta) AS year, MONTH(req_eta) AS month, COALESCE(SUM(orderqty),0) AS qty FROM
        ...
        public static List<OrderWeeklyData> GetCountByWeek_Users(DateTime weekStart, int weekspan)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT Count(order_header.orderid) AS count,Min(porder.po_req_etd) AS req_etd,order_header.userid1,users.user_name,users.customer_code,
        ...
        public static List<OrderWeeklyData> GetCountByWeek_Factories(DateTime weekStart, int weekspan)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT Count(order_header.orderid) AS count,Min(porder.po_req_etd) AS req_etd,porder.userid,factory.user_name,factory.factory_code,
        ...
        public static int? GetTotalQty(string cprod_code1, int company_id)
        {
            ...
            {
                conn.Open();
                var cmd =Utils.GetCommand(@"SELECT SUM(orderqty) FROM
                                    order_header INNER JOIN order_lines ON order_lines.orderid = order_header.orderid
                                    INNER JOIN cust_products ON order_lines.cprod_id = cust_products.cprod_id
        ...
        public static Order_header GetById(int id,bool includeLines = false)
		{
			Order_header result = null;
			using (var conn = new MySqlConnection(Settings.Default.ConnString))
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM order_header WHERE orderid = @id", conn);
                ...
                if (includeLines && result != null)
                    result.Lines = Order_linesDAL.GetByOrderId(id,conn);
            }
			return result;
		}
		...
		public static Order_header GetFromDataReader(MySqlDataReader dr)
		{
		    ...
		    o.order_eb_invoice = Utilities.FromDbValue<int>(dr["eb_invoice"]);
		    o.price_type_override = String.Empty + dr["price_type_override"];

		    if (Utilities.ColumnExists(dr, "po_req_etd"))
		        o.po_req_etd = Utilities.FromDbValue<DateTime>(dr["po_req_etd"]);
            if (Utilities.ColumnExists(dr, "po_factory_id"))
                o.factory_id = Utilities.FromDbValue<int>(dr["po_factory_id"]);
            ...
            if (Utilities.ColumnExists(dr, "instructions"))
                o.po_instructions = string.Empty + dr["instructions"];
		...
		public static void Create(Order_header o)
        {
		    ...
		    try
		    {
		        ...
		        var cmd = Utils.GetCommand("SELECT nextorderid FROM nextorderid", conn, tr);
		...
		public static void Delete(int orderid)
		{
            ...
            {
                conn.Open();
				MySqlCommand cmd = Utils.GetCommand("DELETE FROM order_header WHERE orderid = @id" , conn);
        ...
        public static Order_header GetByCustpo(string custpo)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM order_header WHERE custpo = @custpo", conn);
        ...
        public static List<Company> GetClientsOnOrders(IList<int> factory_ids=null, bool combined = false)
        {
            ...
            using (var conn = new MySqlConnection(Settings.Default.ConnString))
            {
                ...
                result = conn.Query<Company>(sql, new { factory_ids = factory_ids }).ToList();
                if (combined)
        ...
        public static List<Cust_products> GetProductsOnOrders(int? location_id = null,int? factory_id=null, int? client_id=null, string criteria = "", bool spares=false, bool discontinued=false,int? brand_user_id = null, IList<int> factory_ids = null, bool outofstockonly = false, string analysis_d = null, string excluded_distributors = null, int? category1Id = null )
        {
            ...
            using (var conn = new MySqlConnection(Settings.Default.ConnString))
            {
                ...
                var factoryCriteria = factory_ids != null ? string.Format(" mast_products.factory_id IN ({0})", Utils.CreateParametersFromIdList(cmd, factory_ids, "factId"))
                ...
                var analysis_d_criteria = (!string.IsNullOrEmpty(analysis_d) && !discontinued ? $" AND cust_products.analysis_d IN ({Utils.CreateParametersFromIdList(cmd,analysis_d.Split(','),"analysis")}) " : "");
                var distrib_criteria = (!string.IsNullOrEmpty(excluded_distributors) ? $" AND order_header.userid1 NOT IN({Utils.CreateParametersFromIdList(cmd,excluded_distributors.Split(','),"ex_distrib")}) " : "");
                ...
                if (client_id != null)
                {
                    sql = $@"SELECT cust_products.*,mast_products.* FROM cust_products
                                             ...
                                             WHERE {factoryCriteria}
                                                 AND (@location IS NULL OR (order_header.location_override IS NOT NULL AND order_header.location_override = @location)
														OR (order_header.location_override IS NULL AND factory.consolidated_port = @location))
                                                ...
                                                {discont_criteria} {analysis_d_criteria}
                                                AND (mast_products.category1 = @category1 OR @category1 IS NULL)
                ...
                else
                {
                    sql = $@"SELECT mast_products.*,
                            GROUP_CONCAT(cprod_code1 SEPARATOR ', ')  AS cprod_code1,
                            SUM(cprod_stock)  AS cprod_stock,
                            GROUP_CONCAT(cprod_stock_code, ', ') AS cprod_stock_code
                ...
                cmd.Parameters.AddWithValue("@brand_user_id", Utilities.ToDBNull(brand_user_id));
                ...
                cmd.Parameters.AddWithValue("@category1", Utilities.ToDBNull(category1Id));
                //cmd.Parameters.AddWithValue("@analysis_d", "%" + analysis_d + "%");
                var dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    ...
                    {
						var stockCode = Utilities.FromDbValue<int>(dr["cprod_stock_code"]);
						if(stockCode != null)
							stock_codes = new int[] { stockCode.Value }.ToList();
                    }
                    var prod = (client_id != null ? Cust_productsDAL.GetFromDataReader(dr) :
                        new Cust_products{cprod_code1 = string.Empty + dr["cprod_code1"], cprod_stock = Utilities.FromDbValue<int>(dr["cprod_stock"]),cprod_stock_codes = stock_codes, cprod_mast = (int) dr["mast_id"]});
        ...
        public static int GetNumberOfOrders(int cprod_id, DateTime? from = null, DateTime? to = null)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT COUNT(*) FROM order_header WHERE (orderdate >= @from OR @from IS NULL) AND (orderdate <= @to OR @to IS NULL) AND
                                             EXISTS (SELECT cprod_id FROM order_lines WHERE orderid = order_header.orderid AND cprod_id = @cprod_id AND orderqty > 0) AND status NOT IN ('X','Y') AND stock_order = 1", conn);
                cmd.Parameters.AddWithValue("@from", from != null ? (object) from : DBNull.Value);
                cmd.Parameters.AddWithValue("@to", to != null ? (object) to : DBNull.Value);
        ...
        public static List<Order_header> GetByClient(int user_id, string custpo = null)
        {
            var result = new List<Order_header>();
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                conn.Open();
                var query = string.Format("SELECT * FROM order_header WHERE order_header.userid1 = @user_id {0}", !string.IsNullOrEmpty(custpo) ? " AND custpo LIKE @custpo" : "");
        ...
        public static List<Order_header> GetCombinedOrders(int orderid)
        {
            var result = new List<Order_header>();
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString)) {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT * FROM order_header WHERE combined_order = @orderid", conn);
