...
namespace erp.Model.DAL
{
    ...
    public class OrderMgmtDetailDAL
	{
		...
		public static List<OrderMgtmDetail> GetAll()
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM om_detail2", conn);
        ...
        public static List<OrderMgtmDetail> GetByProducts(IList<int> cprod_ids, DateTime eta_from, DateTime eta_to)
        {
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                cmd.CommandText = $@"SELECT * FROM om_detail2 WHERE cprod_id IN ({Utils.CreateParametersFromIdList(cmd, cprod_ids)}) AND stock_order <> 1 
        ...
        public static List<OrderMgtmDetail> GetByMastProduct(int mast_id, DateTime eta_from, DateTime eta_to)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT * FROM om_detail2 INNER JOIN cust_products ON om_detail2.cprod_id = cust_products.cprod_id WHERE cprod_mast = @mast_id AND stock_order <> 1 AND (req_eta BETWEEN @from AND @to OR booked_in_date BETWEEN @from AND @to)", conn);
        ...
        public static List<ProductOrderSummary> GetOrdersTotal(DateTime? from, DateTime? to)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT cprod_id, COALESCE(SUM(orderqty),0) AS TotalQty FROM om_detail1 WHERE (po_req_etd >= @from OR @from IS NULL) AND (po_req_etd <= @to OR @to IS NULL) GROUP BY cprod_id", conn);
                                                        ...
                                                        int orderby)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT om_detail1.* FROM om_detail1 WHERE category1 <> 13
