...
namespace erp.Model.DAL
{
    ...
    public class Stock_order_allocationDAL
	{
		...
		public static List<Stock_order_allocation> GetByProducts(IList<int> cprod_ids)
		{
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                cmd.CommandText = GetSelectSql() + $" WHERE so_lines.cprod_id IN ({Utils.CreateParametersFromIdList(cmd,cprod_ids)})";
        ...
        //    {
        //        conn.Open();
        //        MySqlCommand cmd = Utils.GetCommand("SELECT * FROM stock_order_allocation WHERE unique_link_ref = @id", conn);
        ...
        public static List<soalloc_overallocation> GetOverAllocations()
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT stock_lines.linenum,stock_lines.cprod_id,cust_products.cprod_code1, cust_products.cprod_name,stock_header.custpo,SUM(alloc_qty) AS allocated, stock_lines.orderqty
        ...
        public static List<Order_lines> GetAllocationSOLines(int colineNum)
        {
            var result = new List<Order_lines>();
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                var cmd = Utils.GetCommand(@"SELECT order_lines.*,order_header.custpo,porder_header.po_req_etd, stock_order_allocation.alloc_qty, stock_order_allocation.unique_link_ref
                                        ...
                                        INNER JOIN order_header ON order_lines.orderid = order_header.orderid
                                         INNER JOIN porder_lines ON order_lines.linenum = porder_lines.soline 
	                                     INNER JOIN porder_header ON porder_lines.porderid = porder_header.porderid
                                        WHERE so_line = @coline ORDER BY order_lines.linedate", conn);
                cmd.Parameters.AddWithValue("@coline", colineNum);
                var dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    var line = Order_linesDAL.GetFromDataReader(dr);
        ...
        public static List<Order_lines> GetAvailableStockLines(int cprod_id)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT stock_lines.linenum,stock_header.custpo,porder_header.po_req_etd,stock_lines.orderqty
	                        FROM  
        ...
        public static List<OrderLinesLight> GetOrderLineLightForReport(int? cprod_id)
        {
            var results = new List<OrderLinesLight>();
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                var cmd = Utils.GetCommand(@"
                                            ...
                                            WHERE alloc_qty > 0  AND
	                                            stock_lines.cprod_id = @cprod_id", conn);
                ...
                while (dr.Read())
                {
                    var line = new OrderLinesLight();

                    line.cprod_id = Utilities.FromDbValue<int>(dr["cprod_id"]);
                    line.orderid = Utilities.FromDbValue<int>(dr["orderid"]);
                    line.custpo = string.Empty + dr["custpo"];
                    line.orderqty = Utilities.FromDbValue<int>(dr["orderqty"]);
                    line.calloff_custpo = string.Empty + dr["calloff_custpo"];
                    line.alloc_qty = Utilities.FromDbValue<int>(dr["alloc_qty"]);
        ...
        //    {
        //        conn.Open();
        //        MySqlCommand cmd = Utils.GetCommand("DELETE FROM stock_order_allocation WHERE unique_link_ref = @id" , conn);
