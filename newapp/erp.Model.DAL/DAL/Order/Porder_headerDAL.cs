...
namespace erp.Model.DAL
{
    public class Porder_headerDAL
	{
		...
		public static List<Porder_header> GetAll()
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM porder_header", conn);
		...
		public static Porder_header GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM porder_header WHERE porderid = @id", conn);
		...
		public static void Create(Porder_header o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                MySqlCommand cmd = Utils.GetCommand("SELECT MAX(porderid)+1 FROM porder_header", conn);
        ...
        public static void UpdateSkipLiCreatin(int id, int process_id=170)
        {
            string updatesql = @"UPDATE porder_header SET process_id = @process_id WHERE porderid = @porderid";
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(updatesql, conn);
		...
		public static void Delete(int porderid)
		{
            ...
            {
                conn.Open();
				MySqlCommand cmd = Utils.GetCommand("DELETE FROM porder_header WHERE porderid = @id" , conn);
