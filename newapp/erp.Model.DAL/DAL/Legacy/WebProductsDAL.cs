...
namespace asaq2.Model.DAL
{
    public class WebProductsDAL
    {
        ...
        public static List<WebProduct> GetWebProductsForCat(int catId,int? subCatId = null)
        {
            var result = new List<WebProduct>();
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                var sql = @" SELECT * FROM web_products2 WHERE web_category = @catid  AND COALESCE(web_status,0) <> 2 AND (web_sub_category = @subcat_id OR @subcat_id IS NULL)
                        ORDER BY seq, web_seq";
                var cmd = new MySqlCommand(sql, conn);
                cmd.Parameters.Add(new MySqlParameter("@catid", catId));
                cmd.Parameters.AddWithValue("@subcat_id", Utilities.ToDBNull(subCatId));
                var dr = cmd.ExecuteReader();
                while (dr.Read())
                {
        ...
        public static List<WebProduct> GetAll(string websiteId = null, string language_id = null)
        {
            List<WebProduct> result = new List<WebProduct>();
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                sql += " WHERE (web_site = @site OR @site IS NULL) AND web_status <> 2 ORDER BY web_seq2,sub_seq, web_seq";
                conn.Open();
                MySqlCommand cmd = new MySqlCommand(sql, conn);
                cmd.Parameters.Add(new MySqlParameter("@site", Utilities.ToDBNull(websiteId)));
                if (!string.IsNullOrEmpty(language_id))
                    cmd.Parameters.AddWithValue("@lang", language_id);
                MySqlDataReader dr = cmd.ExecuteReader();
                while (dr.Read())
        ...
        public static WebProduct GetWebProduct(int id, bool includeChildren = false, string language_id = null)
        {
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                sql += " WHERE web_products2.web_unique = @id  AND COALESCE(web_status,0) <> 2 ";
                MySqlCommand cmd = new MySqlCommand(sql, conn);
        ...
        public static List<WebProduct> GetWebProducts(List<int> ids, string language_id = null)
        {
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                string paramstring = Utils.CreateParametersFromIdList(cmd, ids);
        ...
        {
            return GetForBrands(new[] {brand_code}, pattern);
        }

        public static List<WebProduct> GetForBrands(string[] brand_codes, string pattern)
        {
            var result = new List<WebProduct>();
            if (brand_codes != null && brand_codes.Length > 0)
            {
                using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
                {
                    conn.Open();
                    var cmd = new MySqlCommand("", conn);
                    cmd.CommandText = string.Format(@"SELECT web_products2.* FROM web_products2 WHERE web_site IN ({0}) 
                                            AND (web_name LIKE @pattern OR web_code LIKE @pattern OR @pattern = '') ",
                                                    Utils.CreateParametersFromIdList(cmd, brand_codes));
        ...
        public static WebProduct GetProductFromReader(MySqlDataReader dr, bool checkLocalized = true, bool productOnly = false)
        {
            ...
            p.comp1_name= string.Empty + (dr["comp1_name"]);
            p.comp2_name= string.Empty + (dr["comp2_name"]);
            p.comp3_name= string.Empty + (dr["comp3_name"]);
            p.comp4_name= string.Empty + (dr["comp4_name"]);
            p.comp5_name= string.Empty + (dr["comp5_name"]);
            p.comp6_name= string.Empty + (dr["comp6_name"]);
            p.comp7_name= string.Empty + (dr["comp7_name"]);
            p.comp8_name= string.Empty + (dr["comp8_name"]);
            p.comp9_name= string.Empty + (dr["comp9_name"]);
            p.comp10_name= string.Empty + (dr["comp10_name"]);
            p.comp11_name= string.Empty + (dr["comp11_name"]);
            p.comp12_name= string.Empty + (dr["comp12_name"]);

            p.comp1_code = string.Empty + (dr["comp1_code"]);
            p.comp2_code = string.Empty + (dr["comp2_code"]);
            p.comp3_code = string.Empty + (dr["comp3_code"]);
            p.comp4_code = string.Empty + (dr["comp4_code"]);
            p.comp5_code = string.Empty + (dr["comp5_code"]);
            p.comp6_code = string.Empty + (dr["comp6_code"]);
            p.comp7_code = string.Empty + (dr["comp7_code"]);
            p.comp8_code = string.Empty + (dr["comp8_code"]);
            p.comp9_code = string.Empty + (dr["comp9_code"]);
            p.comp10_code = string.Empty + (dr["comp10_code"]);
            p.comp11_code = string.Empty + (dr["comp11_code"]);
            p.comp12_code = string.Empty + (dr["comp12_code"]);
            
            p.comp1_retail = Utilities.FromDbValue<double>(dr["comp1_retail"]);
            p.comp2_retail = Utilities.FromDbValue<double>(dr["comp2_retail"]);
            p.comp3_retail = Utilities.FromDbValue<double>(dr["comp3_retail"]);
            p.comp4_retail = Utilities.FromDbValue<double>(dr["comp4_retail"]);
            p.comp5_retail = Utilities.FromDbValue<double>(dr["comp5_retail"]);
            p.comp6_retail = Utilities.FromDbValue<double>(dr["comp6_retail"]);
            p.comp7_retail = Utilities.FromDbValue<double>(dr["comp7_retail"]);
            p.comp8_retail = Utilities.FromDbValue<double>(dr["comp8_retail"]);
            p.comp9_retail = Utilities.FromDbValue<double>(dr["comp9_retail"]);
            p.comp10_retail = Utilities.FromDbValue<double>(dr["comp10_retail"]);
            p.comp11_retail = Utilities.FromDbValue<double>(dr["comp11_retail"]);
            p.comp12_retail = Utilities.FromDbValue<double>(dr["comp12_retail"]);

            p.prod_nw = Utilities.FromDbValue<int>(dr["prod_nw"]);
            ...
            p.prod_image3 = string.Empty + (dr["prod_image3"]);
            p.comp1_instructions = string.Empty + (dr["comp1_instructions"]);

            p.prod_length = Utilities.FromDbValue<int>(dr["prod_length"]);
            p.prod_width = Utilities.FromDbValue<int>(dr["prod_width"]);
            p.prod_height = Utilities.FromDbValue<int>(dr["prod_height"]);
            ...
            p.inlet02 = Utilities.FromDbValue<double>(dr["inlet02"]);
            p.inlet05 = Utilities.FromDbValue<double>(dr["inlet05"]);
            p.inlet10 = Utilities.FromDbValue<double>(dr["inlet10"]);
            p.inlet20 = Utilities.FromDbValue<double>(dr["inlet20"]);
            p.inlet30 = Utilities.FromDbValue<double>(dr["inlet30"]);
        ...
        private static void BuildSqlParameters(MySqlCommand cmd, WebProduct o, bool forInsert = true)
        {
            ...
            cmd.Parameters.AddWithValue("@inlet02", o.inlet02);
            cmd.Parameters.AddWithValue("@inlet05", o.inlet05);
            cmd.Parameters.AddWithValue("@inlet10", o.inlet10);
            cmd.Parameters.AddWithValue("@inlet20", o.inlet20);
            cmd.Parameters.AddWithValue("@inlet30", o.inlet30);
