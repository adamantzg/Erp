    ...
    public class CategoriesDAL
    {
        ...
        public static List<BrandCategory> GetBrandCategoriesSimple(int brand_id)
        {
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                var cmd = Utils.GetCommand("SELECT brand_categories.*, COALESCE((SELECT COUNT(*) FROM brand_categories_sub WHERE brand_cat_id = brand_categories.brand_cat_id),0) AS childcount FROM brand_categories INNER JOIN brands ON brand_categories.brand = brands.user_id WHERE web_seq > 0 AND brand_id = @brand_id ORDER BY web_seq", conn);
        ...
        public static List<BrandCategory> GetBrandCategories(IList<int> brands = null)
        {
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                cmd.CommandText = string.Format("SELECT * FROM brand_categories INNER JOIN brands ON brand_categories.brand = brands.user_id {0}",brands != null ? 
                    string.Format("WHERE brands.brand_id IN ({0})",Utils.CreateParametersFromIdList(cmd,brands)) : "");
                var dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    result.Add(GetCategoryFromReader(dr));
