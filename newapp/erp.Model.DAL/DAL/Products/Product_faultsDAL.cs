...
namespace erp.Model.DAL
{
    public class Product_faultsDAL
	{
		...
		public static List<Product_faults> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM product_faults", conn);
		...
		public static Product_faults GetById(int id)
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM product_faults WHERE fault_id = @id", conn);
		...
		public static void Delete(int fault_id)
		{
            ...
            {
                conn.Open();
				var cmd = Utils.GetCommand("DELETE FROM product_faults WHERE fault_id = @id" , conn);
        ...
        public static List<Product_faults> GetProductFaults(int cprod_id, DateTime? from, DateTime? to)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM product_faults WHERE fault_cprod = @cprod_id AND (fault_date >= @from OR @from IS NULL) AND (fault_date <= @to OR @to IS NULL)", conn);
...
//                                                AND (fault_date >= @from OR @from IS NULL) AND (fault_date <= @to OR @to IS NULL)",Utilities.CreateParametersFromIdList(cmd,company_ids));
                cmd.CommandText = string.Format(@"SELECT product_faults.*, cust_products.*,mast_products.*,factory.* FROM product_faults INNER JOIN cust_products ON product_faults.fault_cprod = cust_products.cprod_id 
                                                  ...
                                                  WHERE cust_products.cprod_user IN ({0}) AND (fault_date >= @from OR @from IS NULL) AND (fault_date <= @to OR @to IS NULL)",Utils.CreateParametersFromIdList(cmd,company_ids));
                cmd.Parameters.AddWithValue("@from", from != null ? (object)from : DBNull.Value);
                cmd.Parameters.AddWithValue("@to", to != null ? (object)to : DBNull.Value);
                var dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    ...
                    pf.Product.MastProduct.Factory = CompanyDAL.GetFromDataReader(dr);
                    result.Add(pf);
                }
                dr.Close();
            }
