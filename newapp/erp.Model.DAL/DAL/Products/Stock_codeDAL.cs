...
namespace erp.Model.DAL
{
    public class Stock_codeDAL
	{
		...
		public static List<Stock_code> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM stock_code", conn);
		...
		public static Stock_code GetById(int id)
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM stock_code WHERE stock_code_id = @id", conn);
		...
		public static void Create(Stock_code o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                var cmd = Utils.GetCommand("SELECT MAX(stock_code_id)+1 FROM stock_code", conn);
		...
		public static void Delete(int stock_code_id)
		{
            ...
            {
                conn.Open();
				var cmd = Utils.GetCommand("DELETE FROM stock_code WHERE stock_code_id = @id" , conn);
