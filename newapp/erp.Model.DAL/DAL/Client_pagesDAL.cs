...
namespace erp.Model.DAL
{
    public class Client_pagesDAL
	{
		...
		public static List<Client_page> GetAll()
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM client_pages", conn);
		...
		public static Client_page GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM client_pages WHERE page_id = @id", conn);
		...
		public static void Create(Client_page o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                MySqlCommand cmd = Utils.GetCommand("SELECT MAX(page_id)+1 FROM client_pages", conn);
		...
		public static void Delete(int page_id)
		{
            ...
            {
                conn.Open();
				MySqlCommand cmd = Utils.GetCommand("DELETE FROM client_pages WHERE page_id = @id" , conn);
