...
namespace erp.Model.DAL
{
    public class LookupsDal
    {
        ...
        public static IEnumerable<LookupItem> GetLookupItems(string prefix, int count, string tableName, string tableField, string idField = "id")
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(string.Format("SELECT {3} {0},{1} FROM {2} WHERE LEFT({1}, LEN(@prefix)) = @prefix ORDER BY {1}", idField, tableField, tableName, (count == 0 ? "" : string.Format(" TOP {0} ", count))), conn);
        ...
        public static IEnumerable<LookupItem> GetLookupItems(string tableName, string tableField, string idField = "id", string sortField = "")
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(string.Format("SELECT {0},{1} FROM {2} ORDER BY {3}", idField, tableField, tableName, sortField.Length > 0 ? sortField : tableField), conn);
                MySqlDataReader dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    LookupItem li = new LookupItem();
        ...
        public static LookupItem GetLookupItem(string tableName, string tableField, string text, Guid? id = null, string idField="id")
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(string.Format("SELECT {0},{1} FROM {2} WHERE ({1} = @text OR @text IS NULL) AND ({0} = @id OR @id IS NULL)  ORDER BY {1}", idField, tableField, tableName), conn);
