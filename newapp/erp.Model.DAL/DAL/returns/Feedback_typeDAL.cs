...
namespace erp.Model.DAL
{
    public class Feedback_typeDAL
	{
		...
		public static List<Feedback_type> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM feedback_type", conn);
		...
		public static Feedback_type GetById(int id)
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM feedback_type WHERE type_id = @id", conn);
		...
		public static void Create(Feedback_type o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                var cmd = Utils.GetCommand("SELECT MAX(type_id)+1 FROM feedback_type", conn);
		...
		public static void Delete(int type_id)
		{
            ...
            {
                conn.Open();
				var cmd = Utils.GetCommand("DELETE FROM feedback_type WHERE type_id = @id" , conn);
