...
namespace erp.Model.DAL
{
    public class Returns_importanceDAL
	{
		...
		public static List<Returns_importance> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM returns_importance", conn);
        ...
        public static List<Returns_importance> GetForType(int type)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM returns_importance WHERE feedback_type_id = @feedback_type", conn);
		...
		public static Returns_importance GetById(int id)
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM returns_importance WHERE importance_id = @id", conn);
		...
		public static void Create(Returns_importance o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                var cmd = Utils.GetCommand("SELECT MAX(importance_id)+1 FROM returns_importance", conn);
		...
		public static void Delete(int importance_id)
		{
            ...
            {
                conn.Open();
				var cmd = Utils.GetCommand("DELETE FROM returns_importance WHERE importance_id = @id" , conn);
