...
namespace erp.Model.DAL
{
    public class External_userDAL
	{
		...
		public static External_user GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(GetSelectSql() + " WHERE external_user.user_id = @id", conn);
        ...
        public static External_user GetByLoginData(string username, string password = "")
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(GetSelectSql() + " WHERE username = @username" + (password.Length > 0 ? " AND password = @password":""), conn);
		...
		public static void Delete(int user_id)
		{
            ...
            {
                conn.Open();
				MySqlCommand cmd = Utils.GetCommand("DELETE FROM external_user WHERE user_id = @id" , conn);
