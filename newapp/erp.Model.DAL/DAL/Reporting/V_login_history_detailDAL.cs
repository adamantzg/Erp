...
namespace erp.Model.DAL
{
    public class V_login_history_detailDAL
	{
        ...
        {
            return GetByCriteria(new[] {company_id}, dateFrom, dateTo);
        }
	
		public static List<login_history_detail_report> GetByCriteria(IList<int> company_ids, DateTime? dateFrom, DateTime? dateTo)
		{
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                cmd.CommandText = string.Format(@"SELECT * FROM v_login_history_detail WHERE 
                                             (visit_time >= @dateFrom OR @dateFrom IS NULL) AND
                                             (visit_time <= @dateTo OR @dateTo IS NULL) {0}",
                                             company_ids != null && company_ids.Count > 0 ? string.Format(" AND user_id IN ({0})",Utils.CreateParametersFromIdList(cmd,company_ids)) : "");
        ...
        public static List<Company> GetCompanies()
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT DISTINCT users.user_id, user_name, customer_code FROM login_history INNER JOIN users ON login_history.user_id = users.user_id ORDER BY user_name", conn);
                var dr = cmd.ExecuteReader();
                while (dr.Read())
                {
		...
		{
			var o = new login_history_detail_report();
            ...
            //o.userusername = string.Empty + dr["userusername"];
			return o;
