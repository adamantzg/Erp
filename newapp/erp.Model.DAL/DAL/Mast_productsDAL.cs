...
namespace erp.Model.DAL
{
	public class Mast_productsDAL
	{
		...
		public static List<Mast_products> GetAll()
		{
			...
			{
				conn.Open();
				MySqlCommand cmd = Utils.GetCommand("SELECT * FROM mast_products", conn);
		...
		public static Mast_products GetById(int id, bool loadCharacteristics = false)
		{
			...
			{
				conn.Open();
				MySqlCommand cmd = Utils.GetCommand("SELECT * FROM mast_products WHERE mast_id = @id", conn);
		...
		public static Mast_products GetFromDataReader(MySqlDataReader dr)
		{
			...
			if (Utilities.ColumnExists(dr, "category_dop"))
				o.category_dop = Utilities.FromDbValue<int>(dr["category_dop"]);
		    if (Utilities.ColumnExists(dr, "LastPoLinePrice"))
		        o.LastPoLinePrice = Utilities.FromDbValue<double>(dr["LastPoLinePrice"]);
		    if (Utilities.ColumnExists(dr, "speccount"))
		        o.SpecCount = Convert.ToInt32(Utilities.FromDbValue<long>(dr["speccount"]));
            if(Utilities.ColumnExists(dr, "insp_check_override_percent"))
			    o.insp_check_override_percent = Utilities.FromDbValue<double>(dr["insp_check_override_percent"]);
		...
		private static void BuildSqlParameters(MySqlCommand cmd, Mast_products o, bool forInsert = true)
		{
		    ...
		    cmd.Parameters.AddWithValue("@factory_stock", o.factory_stock);
		}
		...
		public static void Delete(int mast_id)
		{
			...
			{
				conn.Open();
				MySqlCommand cmd = Utils.GetCommand("DELETE FROM mast_products WHERE mast_id = @id" , conn);
		...
		public static Mast_products GetByRefAndCode(string factoryRef, string factoryCode)
		{
			...
			{
				conn.Open();
				MySqlCommand cmd = Utils.GetCommand("SELECT * FROM mast_products INNER JOIN users ON mast_products.factory_id = users.user_id WHERE factory_ref = @ref AND users.factory_code = @code", conn);
        ...
        public static List<ProductDate> ProductFirstShipmentDates(IList<int> ids)
        {
            var result = new List<ProductDate>();
            using (var conn = new MySqlConnection(Settings.Default.ConnString)) {
                conn.Open();
                var cmd = new MySqlCommand("", conn);
                cmd.CommandText = string.Format(@"SELECT cust_products.cprod_mast, MIN(porder_header.po_req_etd) AS po_req_etd
                                                ...
                                                WHERE 
                                                order_header.`status` NOT IN ('X','Y') AND porder_lines.orderqty > 0 AND cust_products.cprod_mast IN ({0}) AND order_header.stock_order <> 1
                                                AND (coalesce(`users`.`test_account`,0) = 0)
                                                GROUP BY cust_products.cprod_mast", Utils.CreateParametersFromIdList(cmd, ids));
                var dr = cmd.ExecuteReader();
                while (dr.Read()) {
                    result.Add(new ProductDate { id = (int)dr["cprod_mast"], Date = Utilities.FromDbValue<DateTime>(dr["po_req_etd"]) });
                }
                dr.Close();
            }
            return result;
