    ...
    public class BrochureRequestDAL
    {
        public static void Create(BrochureRequest br)
        {
            string insertsql = @"INSERT INTO brochure_requests (user_id,distributor,user_surname,user_firstname,user_title,user_address1,user_address2,user_address3,user_address4,user_address5,
                                ...
                                user_password,email_date_sent,brand_id,contact_optout) VALUES(@user_id,@distributor,@user_surname,@user_firstname,@user_title,@user_address1,@user_address2,@user_address3,@user_address4,@user_address5,
                                @postcode,@ie_region,@user_country,@user_tel,@user_mobile,@user_email,@user_type,@user_created,@date_sent,@longitude,@latitude,@ip_address,@dealer_id,@lead_source,@sales_status,
                                @user_password,@email_date_sent,@brand_id,@contact_optout)";
        ...
        private static void BuildSqlParameters(MySqlCommand cmd, BrochureRequest br)
        {
            ...
            cmd.Parameters.AddWithValue("@email_date_sent", br.email_date_sent);
            cmd.Parameters.AddWithValue("@brand_id", br.brand_id);
            cmd.Parameters.AddWithValue("@contact_optout", br.contact_optout);
        }
        ...
        public static void Update(BrochureRequest br)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(@"UPDATE brochure_requests SET distributor = @distributor,user_surname = @user_surname,user_firstname = @user_firstname,user_title = @user_title,user_address1 = @user_address1,
                                                        ...
                                                        ip_address = @ip_address,dealer_id = @dealer_id,lead_source = @lead_source,sales_status = @sales_status,user_password = @user_password,email_date_sent = @email_date_sent,brand_id = @brand_id, contact_optout=@contact_optout 
        ...
        public static BrochureRequest GetByCriteria(string email)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM brochure_requests WHERE user_email = @email", conn);
        ...
        public static List<BrochureRequest> GetByDate(DateTime from, DateTime to,bool brand=false)
        {
            var result = new List<BrochureRequest>();
            using(var conn=new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM brochure_requests WHERE user_created >= @from AND user_created <= @to",conn);
                cmd.Parameters.AddWithValue("@from", from);
                cmd.Parameters.AddWithValue("@to", to);
                var dr = cmd.ExecuteReader();
                while(dr.Read())
                {
                   var p=GetBrochureRequestFromReader(dr);

                   if(brand)
                        p.Brand = BrandsDAL.GetById(p.brand_id??0 );
        ...
        public static List<BrochureRequest> GetAll()
        {
            List<BrochureRequest> results = new List<BrochureRequest>();
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM brochure_requests", conn);
                MySqlDataReader dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    results.Add(GetBrochureRequestFromReader(dr));
                }
                dr.Close();
            }
            return results;
        ...
        public static List<BrochureRequest> GetByDealer(int dealer_id)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM brochure_requests WHERE dealer_id = @dealer_id", conn);
        ...
        public static BrochureRequest GetById(int id)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM brochure_requests WHERE user_id = @id", conn);
        ...
        private static BrochureRequest GetBrochureRequestFromReader(MySqlDataReader dr)
        {
            ...
            br.email_date_sent = Utilities.FromDbValue<DateTime>(dr["email_date_sent"]);
            br.brand_id = Utilities.FromDbValue<int>(dr["brand_id"]);
            br.contact_optout = Utilities.FromDbValue<int>(dr["contact_optout"]);
            return br;
        }
    }
}
