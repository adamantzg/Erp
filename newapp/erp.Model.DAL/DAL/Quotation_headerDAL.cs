...
namespace erp.Model.DAL
{
    public class Quotation_headerDAL
	{
		...
		public static Quotation_header GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(GetSelectSql() + " WHERE quotation_id = @id", conn);
        ...
        public static List<Brand> GetBrandsForHeader(int quotation_id)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(@"SELECT DISTINCT brands.* FROM quotation_lines INNER JOIN cust_products ON cust_products.cprod_id = quotation_lines.cprod_id
        ...
        private static List<Quotation_lines> GetLines(int header_id)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(@"SELECT cust_products.*, mast_products.*, users.*, brand_categories.brand FROM quotation_lines INNER JOIN cust_products ON quotation_lines.cprod_id = cust_products.cprod_id 
		...
		public static void Create(Quotation_header o)
        {
			...
			try
            {
                var cmd = Utils.GetCommand(insertsql, conn,trn);
		...
		public static void Update(Quotation_header o)
		{
            ...
            try
            {
                var cmd = Utils.GetCommand(updatesql, conn, trn);
                BuildSqlParameters(cmd, o);
                cmd.ExecuteNonQuery();
		...
		public static void Delete(int quotation_id)
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("DELETE FROM quotation_lines WHERE header_id = @header_id", conn, trn);
                cmd.Parameters.AddWithValue("@header_id", quotation_id);
                cmd.ExecuteNonQuery();
