...
namespace erp.Model.DAL
{
    public class Brand_sales_analysis2DAL
	{
		...
		public static List<Brand_sales_analysis2> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM brand_sales_analysis2", conn);
        ...
        public static List<Brand_sales_analysis2> GetForBrand(int? brand_user_id, DateTime? etdFrom, 
	        bool useETA = false, CountryFilter countryFilter = CountryFilter.UKOnly, string excludedCustomers = "NK2,CWB", OrderDateMode dateMode = OrderDateMode.Etd)
        {
            var result = new List<Brand_sales_analysis2>();
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                {
                    cmd.CommandText = String.Format(@"SELECT brand_sales_analysis2.*, analytics_subcategory.*,analytics_categories.* FROM brand_sales_analysis2
                                            ...
                                            AND ({0} >= @etd OR @etd IS NULL) {1}
                                            AND customer_code NOT IN({2})"
                                            ,dateMode == OrderDateMode.Eta ? "req_eta_nooffset" : dateMode == OrderDateMode.Etd ? "po_req_etd" : "orderdate", AnalyticsDAL.GetCountryCondition(countryFilter), Utils.CreateParametersFromIdList(cmd, excludedCustomers.Split(','), "cust_"));
                ...
                else
                {
                    cmd.CommandText = string.Format(@"SELECT brand_sales_analysis2.*, analytics_subcategory.*,analytics_categories.* FROM brand_sales_analysis2
                                            ...
                                            AND customer_code NOT IN({2})
                                            AND ({0} >= @etd OR @etd IS NULL) {1}", useETA ? "req_eta_nooffset" : "po_req_etd", AnalyticsDAL.GetCountryCondition(countryFilter), Utils.CreateParametersFromIdList(cmd, excludedCustomers.Split(','), "cust_"));
                }
        ...
        public static List<Brand_sales_analysis2> GetForAnalyticsSubcats(DateTime? etdFrom,DateTime? etdTo = null, bool useETA = false, CountryFilter countryFilter = CountryFilter.UKOnly)
        {
            var result = new List<Brand_sales_analysis2>();
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString)) {
                ...
                MySqlCommand cmd;
                var options = Analytics_optionsDAL.GetAll();
                cmd = Utils.GetCommand(string.Format(@"SELECT brand_sales_analysis2.*, analytics_subcategory.*,analytics_categories.* FROM brand_sales_analysis2
                                            INNER JOIN analytics_subcategory ON brand_sales_analysis2.analytics_category = analytics_subcategory.subcat_id 
                                            INNER JOIN analytics_categories ON analytics_subcategory.category_id = analytics_categories.category_id
                                            WHERE ({0} >= @etdFrom OR @etdFrom IS NULL) AND ({0} <= @etdTo OR @etdTo IS NULL) {1} AND cprod_status <> 'D'", useETA ? "req_eta_nooffset" : "po_req_etd", AnalyticsDAL.GetCountryCondition(countryFilter)), conn);
                
                cmd.Parameters.AddWithValue("@etdFrom", Utilities.ToDBNull(etdFrom));
                cmd.Parameters.AddWithValue("@etdTo", Utilities.ToDBNull(etdTo));
                var dr = cmd.ExecuteReader();
                while (dr.Read()) {
                    var bs = GetFromDataReader(dr);
                    if (bs.analytics_category != null) {
        ...
        private static Brand_sales_analysis2 GetFromDataReader(MySqlDataReader dr)
		{
            ...
            o.req_eta_nooffset = Utilities.FromDbValue<DateTime>(dr["req_eta_nooffset"]);
            ...
            o.factory_stock = Utilities.FromDbValue<int>(dr["factory_stock"]);
            o.factory_stock_value = Utilities.FromDbValue<double>(dr["factory_stock_value"]);
