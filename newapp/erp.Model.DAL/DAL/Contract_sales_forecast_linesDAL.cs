...
namespace erp.Model.DAL
{
    public class Contract_sales_forecast_linesDAL
	{
		...
		public static List<Contract_sales_forecast_lines> GetAll()
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM contract_sales_forecast_lines", conn);
        ...
        public static List<Contract_sales_forecast_lines> GetForPeriod(int cprod_id, DateTime dateFrom, DateTime dateTo)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT cf_line.lines_id,cf_line.forecast_id,cf_line.cprod_id,cf_line.qty,cf_line.monthduration,cf.forecast_id,cf.client_id,cf.startmonth,cf.monthduration AS monthduration_header,
        ...
        public static List<Contract_sales_forecast_lines> GetByForecastId(int forecast_id)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT contract_sales_forecast_lines.*, cust_products.cprod_name, cust_products.cprod_code1 AS cprod_code FROM contract_sales_forecast_lines INNER JOIN cust_products ON contract_sales_forecast_lines.cprod_id = cust_products.cprod_id WHERE forecast_id = @forecast_id", conn);
		...
		public static Contract_sales_forecast_lines GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM contract_sales_forecast_lines WHERE lines_id = @id", conn);
		...
		public static void Delete(int lines_id, MySqlTransaction tr)
		{
			...
			MySqlCommand cmd = Utils.GetCommand("DELETE FROM contract_sales_forecast_lines WHERE lines_id = @id" , conn,tr);
        ...
        public static void DeleteByForecast(int forecast_id, MySqlTransaction tr)
        {
            ...
            MySqlCommand cmd = Utils.GetCommand("DELETE FROM contract_sales_forecast_lines WHERE forecast_id = @forecast_id", conn, tr);
            cmd.Parameters.AddWithValue("@forecast_id", forecast_id);
            cmd.ExecuteNonQuery();
