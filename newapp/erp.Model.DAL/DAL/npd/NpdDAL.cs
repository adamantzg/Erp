...
namespace erp.Model.DAL
{
    public class NpdDAL
	{
		...
		public static List<Npd> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT npd.*, (SELECT comments_date FROM npd_comments WHERE npd_id = npd.prod_id ORDER BY comments_date DESC LIMIT 1) AS LastCommentDate, 
        ...
        public static List<Npd> GetByCriteria(int? category1_id, string text)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT npd.*, (SELECT comments_date FROM npd_comments WHERE npd_id = npd.prod_id ORDER BY comments_date DESC LIMIT 1) AS LastCommentDate , npd_status.name AS status_text
        ...
        public static List<Npd> GetForBrand(int brand_id, string text)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT npd.* FROM npd WHERE EXISTS(SELECT npd_id FROM npd_brand WHERE npd_id = npd.prod_id AND brand_id = @brand) AND prod_name LIKE @text", conn);
        ...
        public static int GetNextSequence(DateTime date)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT COALESCE(MAX(month_sequence),0) FROM npd WHERE MONTH(datecreated) = @month", conn);
        ...
        public static Npd GetById(int id)
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT npd.*, category1.cat1_name AS prod_type_text FROM npd LEFT OUTER JOIN category1 ON npd.prod_type = category1.category1_id WHERE prod_id = @id", conn);
		...
		public static void Delete(int prod_id)
		{
            ...
            {
                conn.Open();
				var cmd = Utils.GetCommand("DELETE FROM npd WHERE prod_id = @id" , conn);
