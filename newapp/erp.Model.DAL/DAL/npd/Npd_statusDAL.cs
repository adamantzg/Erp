...
namespace erp.Model.DAL
{
    public class Npd_statusDAL
	{
		...
		public static List<Npd_status> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM npd_status", conn);
		...
		public static Npd_status GetById(int id)
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM npd_status WHERE npdstatus_id = @id", conn);
		...
		public static void Create(Npd_status o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                var cmd = Utils.GetCommand("SELECT MAX(npdstatus_id)+1 FROM npd_status", conn);
		...
		public static void Delete(int npdstatus_id)
		{
            ...
            {
                conn.Open();
				var cmd = Utils.GetCommand("DELETE FROM npd_status WHERE npdstatus_id = @id" , conn);
