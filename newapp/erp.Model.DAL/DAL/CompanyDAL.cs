...
namespace erp.Model.DAL
{
    public class CompanyDAL
    {
        public static List<Company> GetAll()
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(@"SELECT * FROM users ", conn);
        ...
        public static List<Company> GetDistributors(string brand)
        {
            ...
            if (allowedBrands.Contains(brand))
            {
                ...
                {
                    conn.Open();
                    MySqlCommand cmd = Utils.GetCommand(string.Format(@"SELECT users.* FROM users WHERE brand_{0} = 1 AND hide_1 = 0",brand), conn);
                    MySqlDataReader dr = cmd.ExecuteReader();
                    while (dr.Read())
                    {
        ...
        public static List<Company> GetDistributors(bool includeHidden = false)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(@"SELECT users.* FROM users WHERE (distributor>0 OR dealer_distributor> 0) AND (hide_1 = @hide OR @hide IS NULL)", conn);
        ...
        public static List<Company> GetNonBrandClients()
        {
            var result = new List<Company>();

            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString)) {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT users.* FROM users WHERE distributor=0 AND hide_1 = 0", conn);
        ...
        public static List<Company> GetClients(string prefixText = "", IList<Company_User_Type> companyUserType = null )
        {
            List<Company> result = new List<Company>();
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                cmd.CommandText = String.Format(@"SELECT users.* FROM users WHERE (user_type IN ({0}) ) AND (user_name LIKE @criteria OR customer_code LIKE @criteria OR @criteria IS NULL)", Utils.CreateParametersFromIdList(cmd, companyUserType));
        ...
        public static List<Company> GetFactories(string prefixText)
        {
            var result = new List<Company>();

            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString)) {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT users.* FROM users WHERE user_type = 1 AND (user_name LIKE @criteria OR factory_code LIKE @criteria OR @criteria IS NULL)", conn);
                cmd.Parameters.AddWithValue("@criteria", !string.IsNullOrEmpty(prefixText) ? (object)("%" + prefixText + "%") : DBNull.Value);
                var dr = cmd.ExecuteReader();
                while (dr.Read()) {
                    var u = GetFromDataReader(dr);
        ...
        public static List<Company> GetClientsFromProducts()
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT users.* FROM users WHERE user_id IN (SELECT DISTINCT brand_user_id FROM cust_products)", conn);
                var dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    var u = GetFromDataReader(dr);
        ...
        public static List<Company> GetFactoriesForProducts()
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT users.* FROM users WHERE user_type = 1 AND user_id IN (SELECT DISTINCT factory_id FROM mast_products INNER JOIN cust_products ON mast_products.mast_id = cust_products.cprod_mast) ", conn);
                var dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    Company u = GetFromDataReader(dr);
        ...
        public static List<Company> GetFactories()
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT users.* FROM users WHERE user_type = 1", conn);
                var dr = cmd.ExecuteReader();
                while (dr.Read())
                {
                    Company u = GetFromDataReader(dr);
        ...
        public static List<Company> GetFactoriesForLocation(int? location_id = null)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand($@"SELECT users.* FROM users WHERE user_type = 1 AND (consolidated_port = @location_id OR @location_id IS NULL OR EXISTS
						...
						WHERE mast_products.factory_id = users.user_id AND order_header.location_override = @location_id AND order_lines.orderqty > 0))", conn);
        ...
        public static List<Company> GetFactories(IList<int> ids)
        {
            var result = new List<Company>();
        ...
        public static List<Company> GetByIds(List<int> ids)
        {
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString)) {
                conn.Open();
                var cmd = Utils.GetCommand("", conn);
                cmd.CommandText = string.Format("SELECT users.* FROM users WHERE user_id IN ({0})",
                                                Utils.CreateParametersFromIdList(cmd, ids));
                var dr = cmd.ExecuteReader();
                while (dr.Read()) {
                    Company u = GetFromDataReader(dr);
        ...
        public static List<Company> GetCompaniesForUser(int userid, Company_User_Type type = Company_User_Type.Factory)
        {
            var result = new List<Company>();
        ...
        public static Company GetById(int id)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM users WHERE user_id = @id", conn);
        ...
        public static Company GetByFactoryCode(string factory_code)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM users WHERE factory_code = @fcode", conn);
        ...
        public static Company GetFactoryForProduct(int cprod_id)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(@"SELECT users.* FROM cust_products INNER JOIN mast_products ON mast_products.mast_id = cust_products.cprod_mast
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                cmd.CommandText =
                    string.Format(@"SELECT DISTINCT users.* FROM cust_products INNER JOIN mast_products ON mast_products.mast_id = cust_products.cprod_mast
                                  ...
                                  Utils.CreateParametersFromIdList(cmd, client_ids));
        ...
        public static List<Company> GetCompaniesByType(Company_User_Type user_type)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM users WHERE user_type = @user_type", conn);
        ...
        public static List<SiteEdit> GetSiteEdits(int company_id)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(@"SELECT brands.brand_id,brands.brandname,brands.user_id,brands.`code`,brands.dealerstatus_view,brands.image,user_brand_lang.language_id,
        ...
        public static Company GetFromDataReader(MySqlDataReader dr)
	    {
          ...
          //  o.order_id = (int)(dr["orderid"]);
            ...
            o.brand_ar = Utilities.FromDbValue<int>(dr["brand_ar"]);
            o.brand_ac = Utilities.FromDbValue<int>(dr["brand_ac"]);
	        ...
	        o.EF_fix_2011_interface = Utilities.FromDbValue<int>(dr["2011_interface"]);
            ...
            o.stock_order_month_limit = Utilities.FromDbValue<int>(dr["stock_order_month_limit"]);
            o.invoice_sequence = Utilities.FromDbValue<int>(dr["invoice_sequence"]);
            o.invoice_logo = string.Empty + dr["invoice_logo"];
            o.test_account = Utilities.FromDbValue<int>(dr["test_account"]);
            return o;
    	}
        ...
        public static List<Company> GetAllSiblings(int companyId)
        {
            ...
            if (company != null)
            {
                ...
                {
                    conn.Open();
                    var cmd = Utils.GetCommand("SELECT * FROM users WHERE combined_factory = @id", conn);
        ...
        public static List<Company> GetFactoriesByCombinedCode(int combined_factory)
        {
            List<Company> result = new List<Company>();
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString)) {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM users WHERE combined_factory = @id", conn);
                cmd.Parameters.AddWithValue("@id", combined_factory);
                var dr = cmd.ExecuteReader();
                while (dr.Read()) {
                    result.Add(GetFromDataReader(dr));
                }
        ...
        public static Company GetByCustomerCode(string customerCode)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM users WHERE customer_code = @code", conn);
