...
namespace erp.Model.DAL
{
    public class Invoice_linesDAL
	{
		...
		public static List<Invoice_lines> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM invoice_lines", conn);
		...
		public static Invoice_lines GetById(int id)
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM invoice_lines WHERE linenum = @id", conn);
		...
		public static void Create(Invoice_lines o,IDbTransaction tr = null)
        {
            string insertsql = @"INSERT INTO invoice_lines (invoice_id,linedate,cprod_id,description,orderqty,unitprice,unitcurrency,linestatus,record_type,qty_type,image_id) 
                    VALUES(@invoice_id,@linedate,@cprod_id,@description,@orderqty,@unitprice,@unitcurrency,@linestatus,@record_type,@qty_type,@image_id)";


		    var conn = (tr != null ? (tr as MySqlTransaction).Connection : new MySqlConnection(Properties.Settings.Default.ConnString));
            if(tr == null)
                conn.Open();
		...
		public static void Update(Invoice_lines o, MySqlTransaction tr = null)
		{
			string updatesql = @"UPDATE invoice_lines SET invoice_id = @invoice_id,linedate = @linedate,cprod_id = @cprod_id,description = @description,orderqty = @orderqty,unitprice = @unitprice,
                    unitcurrency = @unitcurrency,linestatus = @linestatus,record_type = @record_type,qty_type = @qty_type, image_id=@image_id WHERE linenum = @linenum";
		...
		public static void Delete(int linenum, IDbTransaction tr = null)
		{
			var conn = tr != null ? (MySqlConnection) tr.Connection : new MySqlConnection(Properties.Settings.Default.ConnString);
            if(tr == null)
                conn.Open();
			var cmd = Utils.GetCommand("DELETE FROM invoice_lines WHERE linenum = @id" , conn,(MySqlTransaction) tr);
        ...
        public static List<Invoice_lines> GetByInvoice(int id)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM invoice_lines WHERE invoice_id = @invoice", conn);
