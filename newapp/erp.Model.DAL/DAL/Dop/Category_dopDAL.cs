...
namespace erp.Model.DAL
{
    public class Category_dopDAL
	{
		...
		public static List<Category_dop> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM category_dop", conn);
		...
		public static Category_dop GetById(int id)
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM category_dop WHERE category_id = @id", conn);
		...
		public static void Create(Category_dop o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                MySqlCommand cmd = Utils.GetCommand("SELECT MAX(category_id)+1 FROM category_dop", conn);
		...
		public static void Delete(int category_id)
		{
            ...
            {
                conn.Open();
				var cmd = Utils.GetCommand("DELETE FROM category_dop WHERE category_id = @id" , conn);
