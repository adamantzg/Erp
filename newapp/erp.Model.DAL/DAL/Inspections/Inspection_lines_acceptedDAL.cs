...
namespace erp.Model.DAL
{
    public class Inspection_lines_acceptedDAL
	{
		...
		public static List<Inspection_lines_accepted> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM inspection_lines_accepted", conn);
        ...
        public static List<Inspection_lines_accepted> GetByInspection(int insp_id)
        {
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT * FROM inspection_lines_accepted INNER JOIN inspection_lines_tested ON inspection_lines_accepted.insp_line_id = inspection_lines_tested.insp_line_unique
                                            WHERE inspection_lines_tested.insp_id = @insp_id", conn);
        ...
        public static List<Inspection_lines_accepted> GetByInspectionBasic(int insp_id)
        {
            var result = new List<Inspection_lines_accepted>();
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                conn.Open();
                var cmd = Utils.GetCommand(@"SELECT * FROM inspection_lines_accepted 
                                            WHERE inspection_lines_accepted.insp_unique = @insp_id", conn);
                cmd.Parameters.AddWithValue("@insp_id", insp_id);
                var dr = cmd.ExecuteReader();
                while (dr.Read())
                {
		...
		public static Inspection_lines_accepted GetById(int id)
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM inspection_lines_accepted WHERE insp_line_unique = @id", conn);
		...
		public static void Delete(int insp_line_unique)
		{
            ...
            {
                conn.Open();
				var cmd = Utils.GetCommand("DELETE FROM inspection_lines_accepted WHERE insp_line_unique = @id" , conn);
