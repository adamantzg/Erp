...
namespace erp.Model.DAL
{
    public class Dealer_brandstatusDAL
	{
		...
		public static List<Dealer_brandstatus> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM dealer_brandstatus", conn);
		...
		public static Dealer_brandstatus GetById(int brand_id, int dealer_id)
		{
            ...
            {
                conn.Open();
                var cmd = Utils.GetCommand("SELECT * FROM dealer_brandstatus WHERE brand_id = @brand_id AND dealer_id = @dealer_id", conn);
		...
		public static void Delete(int brand_id, int dealer_id)
		{
            ...
            {
                conn.Open();
				var cmd = Utils.GetCommand("DELETE FROM dealer_brandstatus WHERE brand_id = @brand_id AND dealer_id = @dealer_id" , conn);
                cmd.Parameters.AddWithValue("@brand_id", brand_id);
                cmd.Parameters.AddWithValue("@dealer_id", dealer_id);
                cmd.ExecuteNonQuery();
            }
