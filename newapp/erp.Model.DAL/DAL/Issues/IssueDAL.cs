...
namespace erp.Model.DAL
{
    public class IssueDAL
	{
		...
		public static Issue GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(GetIssueSQL() + " WHERE issue_id = @id", conn);
        ...
        public static bool Validate(Issue i, out string message)
        {
            ...
            if (i.area_id == null)
            {
                message = "Area must be specified.";
                valid = false;
                goto Exit;
            }
        Exit:
		...
		public static void Delete(int issue_id)
		{
            ...
            try
            {
                ...
                conn.Open();
                tr = conn.BeginTransaction();
                MySqlCommand cmd = Utils.GetCommand("DELETE FROM issue_categories WHERE issue_id = @id", conn, tr);
