...
namespace erp.Model.DAL
{
    public class Issue_typeDAL
	{
		...
		public static List<Issue_type> GetAll()
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM issue_type", conn);
		...
		public static Issue_type GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM issue_type WHERE type_id = @id", conn);
		...
		public static void Create(Issue_type o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                MySqlCommand cmd = Utils.GetCommand("SELECT MAX(type_id)+1 FROM issue_type", conn);
		...
		public static void Delete(int issuetype_id)
		{
            ...
            {
                conn.Open();
				MySqlCommand cmd = Utils.GetCommand("DELETE FROM issue_type WHERE type_id = @id" , conn);
