...
namespace erp.Model.DAL
{
    public class Issue_categoryDAL
	{
		...
		public static List<Issue_category> GetAll()
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM issue_category", conn);
        ...
        public static List<Issue_category> GetCategoriesForArea(int area_id)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(@"SELECT issue_category.* FROM issuecategory_areas INNER JOIN issue_category ON issue_category.category_id = issuecategory_areas.category_id
		...
		public static Issue_category GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM issue_category WHERE category_id = @id", conn);
        ...
        public static List<Issue_category> GetCategoriesForIssue(int issue_id, int? parent_id = null)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand(@"SELECT issue_category.* FROM issue_categories INNER JOIN issue_category ON issue_category.category_id = issue_categories.category_id
        ...
        private static bool CanBeDeleted(int category_id, ref List<Issue> issues, MySqlConnection conn)
        {
            ...
            MySqlCommand cmd = Utils.GetCommand(@"SELECT issue.issue_id, issue.title FROM issue_categories  INNER JOIN issue ON issue_categories.issue_id = issue.issue_id 
        ...
        public static void Create(Issue_category o)
        {
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                MySqlCommand cmd = Utils.GetCommand("SELECT COALESCE(MAX(category_id),0)+1 FROM issue_category", conn);
		...
		public static void Delete(int category_id)
		{
            ...
            {
                conn.Open();
				MySqlCommand cmd = Utils.GetCommand("DELETE FROM issue_category WHERE category_id = @id" , conn);
        ...
        public static void UpdateAreas(int id, int[] areas)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("DELETE FROM issuecategory_areas WHERE category_id = @id", conn);
                cmd.Parameters.AddWithValue("@id", id);
                cmd.ExecuteNonQuery();
