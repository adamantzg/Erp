...
namespace erp.Model.DAL
{
    public class Issue_priorityDAL
	{
		...
		public static List<Issue_priority> GetAll()
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM issue_priority", conn);
		...
		public static Issue_priority GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM issue_priority WHERE priority_id = @id", conn);
		...
		public static void Create(Issue_priority o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                MySqlCommand cmd = Utils.GetCommand("SELECT MAX(priority_id)+1 FROM issue_priority", conn);
		...
		public static void Delete(int priority_id)
		{
            ...
            {
                conn.Open();
				MySqlCommand cmd = Utils.GetCommand("DELETE FROM issue_priority WHERE priority_id = @id" , conn);
