...
namespace erp.Model.DAL
{
    public class Issuecomment_filesDAL
	{
		...
		public static List<Issuecomment_files> GetForComment(int comment_id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM issuecomment_files WHERE comment_id = @comment_id", conn);
		...
		public static Issuecomment_files GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = Utils.GetCommand("SELECT * FROM issuecomment_files WHERE id = @id", conn);
