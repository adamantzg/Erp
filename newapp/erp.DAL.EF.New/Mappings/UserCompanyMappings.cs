...
namespace erp.DAL.EF.Mappings
{
    ...
    public class CompanyMappings : EntityTypeConfiguration<Company>
    {
        public CompanyMappings()
        {
			...
			HasMany(c=>c.Files).WithMany(f=>f.Companies).Map(m=>m.ToTable("users_file").MapLeftKey("user_id").MapRightKey("file_id"));
			HasMany(c=>c.CustProducts).WithOptional().HasForeignKey(p=>p.brand_userid);
        }
    }
