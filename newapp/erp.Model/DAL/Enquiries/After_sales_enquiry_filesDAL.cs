...
namespace asaq2.Model
{
    public class After_sales_enquiry_filesDAL
	{
		...
		public static List<After_sales_enquiry_files> GetForEnquiry(int enquiry_id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM after_sales_enquiry_files WHERE enquiry_id = @enquiry_id ", conn);
		...
		public static After_sales_enquiry_files GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM after_sales_enquiry_files WHERE file_id = @id", conn);
        ...
        public static void Delete(int file_id)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("DELETE FROM after_sales_enquiry_files WHERE file_id = @id", conn);
