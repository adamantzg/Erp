...
namespace asaq2.Model
{
    public class After_sales_enquiry_statusDAL
	{
		...
		public static List<After_sales_enquiry_status> GetAll()
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM after_sales_enquiry_status", conn);
		...
		public static After_sales_enquiry_status GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM after_sales_enquiry_status WHERE status_id = @id", conn);
		...
		public static void Create(After_sales_enquiry_status o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                MySqlCommand cmd = new MySqlCommand("SELECT MAX(status_id)+1 FROM after_sales_enquiry_status", conn);
		...
		public static void Delete(int status_id)
		{
            ...
            {
                conn.Open();
				MySqlCommand cmd = new MySqlCommand("DELETE FROM after_sales_enquiry_status WHERE status_id = @id" , conn);
