...
namespace asaq2.Model
{
    public class Quotation_companiesDAL
	{
		...
		public static Quotation_companies GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand(GetSelectSql() + " WHERE company_id = @id", conn);
		...
		public static void Delete(int company_id)
		{
            ...
            {
                conn.Open();
				MySqlCommand cmd = new MySqlCommand("DELETE FROM quotation_companies WHERE company_id = @id" , conn);
