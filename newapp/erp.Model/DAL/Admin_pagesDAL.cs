...
namespace asaq2.Model
{
    public class Admin_pagesDAL
	{
		...
		public static List<Admin_pages> GetAll()
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM admin_pages", conn);
        ...
        public static List<Admin_pages> GetForUser(int user_id)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT admin_pages.* FROM admin_pages INNER JOIN user_pages ON admin_pages.page_id = user_pages.page_id WHERE userid = @user_id", conn);
		...
		public static Admin_pages GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM admin_pages WHERE page_id = @id", conn);
		...
		public static void Create(Admin_pages o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                MySqlCommand cmd = new MySqlCommand("SELECT MAX(page_id)+1 FROM admin_pages", conn);
		...
		public static void Delete(int page_id)
		{
            ...
            {
                conn.Open();
				MySqlCommand cmd = new MySqlCommand("DELETE FROM admin_pages WHERE page_id = @id" , conn);
