...
namespace asaq2.Model
{
    public class Sales_forecastDAL
	{
		...
		public static List<Sales_forecast> GetAll()
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM sales_forecast", conn);
        ...
        public static List<Sales_forecast> GetForecastForPeriod(int monthFrom, int monthTo, List<int> numCprodUser)
        {
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                cmd.CommandText = string.Format("SELECT * FROM sales_forecast " +
                                                ...
                                                numCprodUser != null ? string.Format(" cprod_user IN ({0})",Utilities.CreateParametersFromIdList(cmd,numCprodUser)):null);
        ...
        public static List<Sales_forecast> GetForPeriod( int cprod_id,int monthFrom, int monthTo)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM sales_forecast WHERE cprod_id = @cprod_id AND month21 BETWEEN @from AND @to", conn);
        ...
        public static List<Sales_forecast> GetForPeriod(IList<int> cprod_ids, int monthFrom, int monthTo)
        {
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                cmd.CommandText =
                    string.Format(
                        "SELECT * FROM sales_forecast WHERE cprod_id IN ({0}) AND month21 BETWEEN @from AND @to",
        ...
        public static List<Sales_forecast> GetForMastProdAndPeriod(int mast_id, int monthFrom, int monthTo)
        {
            List<Sales_forecast> result = new List<Sales_forecast>();
            using (var conn = new MySqlConnection(Settings.Default.ConnString))
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand(@"SELECT * FROM sales_forecast INNER JOIN cust_products ON sales_forecast.cprod_id = cust_products.cprod_id 
		...
		public static Sales_forecast GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM sales_forecast WHERE sales_unique = @id", conn);
		...
		public static void Delete(int sales_unique)
		{
            ...
            {
                conn.Open();
				MySqlCommand cmd = new MySqlCommand("DELETE FROM sales_forecast WHERE sales_unique = @id" , conn);
        ...
        public static void DeleteByIdAndMonth(int cprod_id, int? monthFrom = null, int? monthTo = null)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("DELETE FROM sales_forecast WHERE cprod_id = @cprod_id AND (month21>= @monthFrom OR @monthFrom IS NULL) AND (month21<= @monthTo OR @monthTo IS NULL)", conn);
                cmd.Parameters.AddWithValue("@cprod_id", cprod_id);
