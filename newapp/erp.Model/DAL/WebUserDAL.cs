...
namespace asaq2.Model
{
    public class WebUserDAL
    {
        public static WebUser GetUser(string email, string password)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM brochure_requests WHERE user_email = @email AND user_password = @password", conn);
        ...
        public static bool EmailExists(string email)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT COUNT(*) FROM brochure_requests WHERE user_email = @email", conn);
                cmd.Parameters.Add(new MySqlParameter("@email", email));
        ...
        public static long Create(WebUser user)
        {
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                cmd = new MySqlCommand("INSERT INTO brochure_requests(user_id, user_email, user_password) VALUES(@id,@email,@password)", conn);
