...
namespace asaq2.Model
{
    public class DealerDAL
    {
        ...
        /// <returns></returns>
        public static List<Dealer> GetNearestDealers(string website, double latitude, double longitude,int? web_unique = null)
        { 
...
//            return result;
            Brand b = BrandsDAL.GetByCode(website);
            var dealers = GetDealersForBrand(website,web_unique);
...
//            return result;
        ...
        public static List<Dealer> GetDealersForBrand(string brand_code, int? web_unique = null)
        {
            var result = new List<Dealer>();
            Brand b = BrandsDAL.GetByCode(brand_code);
            if (b != null )
                ...
                using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
                {
                    ...
                    if (!string.IsNullOrEmpty(b.dealerstatus_view))
                    {
                        cmd.CommandText = string.Format(@"SELECT {0}.*,dist.user_name as dist_name, dist.customer_code As dist_code, 
                                                        (SELECT COUNT(*) FROM dealer_images WHERE dealer_id = {0}.user_id) AS numImages FROM {0} 
                                                        INNER JOIN users AS dist ON {0}.user_type = dist.user_id {1}",b.dealerstatus_view,string.Format(productCondition,b.dealerstatus_view,"WHERE"));
                    }
                    else
                    {
                        ...
                        WHERE sales_registered=1 AND ( EXISTS(SELECT unique_id FROM dealer_displays INNER JOIN web_products ON dealer_displays.web_unique = web_products.web_unique 
                          ...
                          default_brand = @brand_id ) {0}",string.Format(productCondition,"dealers"," AND "));
        ...
        public static int GetNonDisplayingDealersCount(int? user_type = null)
        {
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                var cmd =
                    new MySqlCommand(
                        @"SELECT COUNT(*) FROM dealers WHERE NOT EXISTS (SELECT * FROM dealer_displays WHERE client_id = dealers.user_id) 
                            AND NOT EXISTS(SELECT dealer_image_displays.image_id FROM dealer_image_displays INNER JOIN dealer_images ON dealer_image_displays.image_id = dealer_images.image_unique WHERE dealer_id = dealers.user_id)
                            AND (dealers.user_type = @user_type OR @user_type IS NULL)", conn);
        ...
        public static Dealer GetById(int id)
        {
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                if (dr.Read())
                {
                    ...
                    var imageDisplays = Dealer_image_displaysDAL.GetForDealer(id);
        ...
        public static void Create(Dealer o)
        {
            string insertsql = @"INSERT INTO dealers(user_id,user_name,user_account,customer_code,distributor,user_welcomename,user_address1,user_address2,
                                ...
                                brand_b,brand_wc,confirmed,default_brand,sales_registered,user_modified) 
                                ...
                                @image_policy_ip,@training,@training_date,@brand_b,@brand_wc,@confirmed,@default_brand,@sales_registered,@user_modified)";
        ...
        public static void Update(Dealer d, List<int> deletedImagesIds=null, List<int> deletedProducts=null)
        {
            string updatesql = @"UPDATE dealers SET user_name = @user_name,user_account = @user_account,customer_code = @customer_code,distributor = @distributor,
                                ...
                                sales_registered = @sales_registered,user_modified = @user_modified WHERE user_id = @user_id";
        ...
        public static List<Dealer> GetDealersProcessedInMonth(DateTime month, int status)
        {
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                var cmd = new MySqlCommand(@"SELECT dealers.*, users.customer_code AS dist_code,
                                                (SELECT COUNT(*) FROM dealer_images WHERE dealer_id = dealers.user_id) AS numImages 
                                            FROM dealers INNER JOIN users ON dealers.user_type = users.user_id 
                                            WHERE ( COALESCE(dealers.hide_1,0) = @status) AND dealers.confirmed = 1 AND dealers.hide_1 <> 2 AND (dealers.user_created BETWEEN @from AND @to OR dealers.user_modified BETWEEN @from AND @to)", conn);
        ...
        public static List<Dealer> GetDealersByCriteria(string searchText)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand(@"SELECT dealers.* FROM  dealers WHERE (user_name LIKE @criteria OR user_email LIKE @criteria) AND COALESCE(hide_1,0) <> 2", conn);
                //cmd.Parameters.AddWithValue("@user_type", distributor_id);
        ...
        public static void Delete(int id)
        {
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("DELETE FROM dealers WHERE user_id = @id", conn);
                cmd.Parameters.AddWithValue("@id", id);
                cmd.ExecuteNonQuery();
            }
        }
        ...
        public static void DeleteDisplays(int dealer_id)
        {
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("DELETE FROM dealer_displays WHERE client_id = @id", conn);
