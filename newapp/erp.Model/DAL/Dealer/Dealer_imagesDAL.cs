...
namespace asaq2.Model
{
    public class Dealer_imagesDAL
	{
		...
		public static void Create(Dealer_images o, DbTransaction tr)
        {
			...
			var cmd = new MySqlCommand(insertsql, conn,(MySqlTransaction) tr);
		...
		public static void Delete(int image_unique, DbTransaction tr)
		{
            ...
            if (tr == null)
                conn.Open();
            var cmd = new MySqlCommand("SELECT dealer_id FROM dealer_images WHERE image_unique = @id", (MySqlConnection)conn, (MySqlTransaction)tr);
