...
namespace asaq2.Model
{
    public class Issue_areaDAL
	{
		...
		public static List<Issue_area> GetAll()
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM issue_area", conn);
		...
		public static Issue_area GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM issue_area WHERE area_id = @id", conn);
        ...
        public static List<Issue_area> GetForCategory(int category_id)
        {
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT issue_area.* FROM issuecategory_areas INNER JOIN issue_area ON issuecategory_areas.area_id = issue_area.area_id WHERE category_id = @category_id", conn);
        ...
        public static void Create(Issue_area o)
        {
            ...
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                MySqlCommand cmd = new MySqlCommand("SELECT MAX(area_id)+1 FROM issue_area", conn);
		...
		public static void Delete(int area_id)
		{
            ...
            {
                conn.Open();
				MySqlCommand cmd = new MySqlCommand("DELETE FROM issue_area WHERE area_id = @id" , conn);
