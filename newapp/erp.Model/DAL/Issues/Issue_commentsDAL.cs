...
namespace asaq2.Model
{
    public class Issue_commentsDAL
	{
		...
		public static List<Issue_comments> GetForIssue(int issue_id, bool includeFiles = true)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand(@"SELECT issue_comments.*,  Creator.userwelcome AS creator
		...
		public static Issue_comments GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM issue_comments WHERE comment_id = @id", conn);
