...
namespace asaq2.Model
{
    ...
    {
        public static ContactRequest GetById(int id, string language_id = null)
        { 
            ContactRequest c = null;
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM contact_requests WHERE user_id = @id", conn);
                ...
                if (dr.Read())
                {
                    ...
                    c.Products = WebProductsDAL.GetWebProducts(productids, language_id);
                }
                dr.Close();
            }
            return c;
        ...
        public static void Create(ContactRequest o)
        {
            ...
            try
            {
                MySqlCommand cmd = new MySqlCommand("SELECT COALESCE(MAX(user_id),0)+1 FROM contact_requests", conn);
