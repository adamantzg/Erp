...
namespace asaq2.Model
{
    public class Creditnote_lineDAL
	{
		...
		public static List<Creditnote_line> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM creditnote_line", conn);
        ...
        public static List<Creditnote_line> GetForInvoice(int id)
        {
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM creditnote_line WHERE invoice_id = @id", conn);
		...
		public static Creditnote_line GetById(int id)
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM creditnote_line WHERE line_id = @id", conn);
				cmd.Parameters.AddWithValue("@id", id);
                var dr = cmd.ExecuteReader();
                if (dr.Read())
                {
        ...
        {
            string insertsql = @"INSERT INTO creditnote_line (invoice_id,return_no,cprod_code,client_ref,cprod_name,unitprice,quantity) VALUES(@invoice_id,@return_no,@cprod_code,@client_ref,@cprod_name,@unitprice,@quantity)";
		...
		{
			string updatesql = @"UPDATE creditnote_line SET invoice_id = @invoice_id,return_no = @return_no,cprod_code = @cprod_code,client_ref = @client_ref,cprod_name = @cprod_name,unitprice = @unitprice,quantity = @quantity WHERE line_id = @line_id";
		...
		public static void Delete(int line_id)
		{
            ...
            {
                conn.Open();
				var cmd = new MySqlCommand("DELETE FROM creditnote_line WHERE line_id = @id" , conn);
