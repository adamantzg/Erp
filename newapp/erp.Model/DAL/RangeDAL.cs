...
namespace asaq2.Model
{
    public class RangeDAL
	{
		...
		public static List<Range> GetAll()
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM ranges", conn);
		...
		public static Range GetById(int id)
		{
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM ranges WHERE rangeid = @id", conn);
        ...
        public static List<Range> GetByCompanyId(int id, bool combined = true)
        {
            ...
            if (company != null)
            {
                using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
                {
                    ...
                    var cmd =
                        new MySqlCommand(string.Format(@"SELECT ranges.* FROM ranges WHERE LENGTH(TRIM(range_desc)) > 0 AND  rangeid 
		...
		public static void Create(Range o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                MySqlCommand cmd = new MySqlCommand("SELECT MAX(rangeid)+1 FROM ranges", conn);
		...
		public static void Delete(int rangeid)
		{
            ...
            {
                conn.Open();
				MySqlCommand cmd = new MySqlCommand("DELETE FROM ranges WHERE rangeid = @id" , conn);
