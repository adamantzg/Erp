...
namespace asaq2.Model
{
    public class Productfault_reason_descriptionDAL
	{
		...
		public static List<Productfault_reason_description> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM productfault_reason_description", conn);
		...
		public static Productfault_reason_description GetById(int id)
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM productfault_reason_description WHERE xpgcode = @id", conn);
		...
		public static void Delete(int xpgcode)
		{
            ...
            {
                conn.Open();
				var cmd = new MySqlCommand("DELETE FROM productfault_reason_description WHERE xpgcode = @id" , conn);
