...
namespace asaq2.Model
{
    public class Feedback_categoryDAL
	{
		...
		public static List<Feedback_category> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM feedback_category", conn);
        ...
        public static List<Feedback_category> GetForType(int feedback_type)
        {
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM feedback_category WHERE feedback_type = @feedback_type", conn);
		...
		public static Feedback_category GetById(int id)
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM feedback_category WHERE feedback_cat_id = @id", conn);
		...
		public static void Create(Feedback_category o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                var cmd = new MySqlCommand("SELECT MAX(feedback_cat_id)+1 FROM feedback_category", conn);
		...
		public static void Delete(int feedback_cat_id)
		{
            ...
            {
                conn.Open();
				var cmd = new MySqlCommand("DELETE FROM feedback_category WHERE feedback_cat_id = @id" , conn);
