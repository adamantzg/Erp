...
namespace asaq2.Model
{
    public class Return_categoryDAL
	{
		...
		public static List<Return_category> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM return_category", conn);
		...
		public static Return_category GetById(int id)
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM return_category WHERE returncategory_id = @id", conn);
		...
		public static void Create(Return_category o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                MySqlCommand cmd = new MySqlCommand("SELECT MAX(returncategory_id)+1 FROM return_category", conn);
		...
		public static void Delete(int returncategory_id)
		{
            ...
            {
                conn.Open();
				var cmd = new MySqlCommand("DELETE FROM return_category WHERE returncategory_id = @id" , conn);
