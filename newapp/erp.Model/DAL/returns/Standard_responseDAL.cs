...
namespace asaq2.Model
{
    public class Standard_responseDAL
	{
		...
		public static List<Standard_response> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM standard_response", conn);
		...
		public static Standard_response GetById(int id)
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM standard_response WHERE response_id = @id", conn);
		...
		public static void Create(Standard_response o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                var cmd = new MySqlCommand("SELECT MAX(response_id)+1 FROM standard_response", conn);
		...
		public static void Delete(int response_id)
		{
            ...
            {
                conn.Open();
				var cmd = new MySqlCommand("DELETE FROM standard_response WHERE response_id = @id" , conn);
