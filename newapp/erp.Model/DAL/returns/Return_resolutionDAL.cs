...
namespace asaq2.Model
{
    public class Return_resolutionDAL
	{
		...
		public static List<Return_resolution> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM return_resolution", conn);
		...
		public static Return_resolution GetById(int id)
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM return_resolution WHERE id = @id", conn);
		...
		public static void Create(Return_resolution o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                MySqlCommand cmd = new MySqlCommand("SELECT MAX(id)+1 FROM return_resolution", conn);
		...
		public static void Delete(int id)
		{
            ...
            {
                conn.Open();
				var cmd = new MySqlCommand("DELETE FROM return_resolution WHERE id = @id" , conn);
                cmd.Parameters.AddWithValue("@id", id);
                cmd.ExecuteNonQuery();
            }
		}
