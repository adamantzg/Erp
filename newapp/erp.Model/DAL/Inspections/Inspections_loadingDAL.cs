...
namespace asaq2.Model
{
    public class Inspections_loadingDAL
	{
		...
		public static List<Inspections_loading> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM inspections_loading", conn);
        ...
        public static List<Inspections_loading> GetForInspection(int insp_id)
        {
            var result = new List<Inspections_loading>();
            using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                conn.Open();
                var cmd = new MySqlCommand(@"SELECT inspections_loading.*,inspection_lines_tested.*  FROM inspections_loading INNER JOIN inspection_lines_tested 
                                             ON inspections_loading.insp_line_unique = inspection_lines_tested.insp_line_unique 
                                            INNER JOIN containers ON inspections_loading.container = containers.container_id WHERE containers.insp_id = @insp_id", conn);
		...
		public static Inspections_loading GetById(int id)
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM inspections_loading WHERE loading_line_unique = @id", conn);
		...
		public static void Delete(int loading_line_unique)
		{
            ...
            {
                conn.Open();
				var cmd = new MySqlCommand("DELETE FROM inspections_loading WHERE loading_line_unique = @id" , conn);
