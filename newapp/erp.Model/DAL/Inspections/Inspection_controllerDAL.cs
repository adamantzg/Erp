...
namespace asaq2.Model
{
    public class Inspection_controllerDAL
	{
		...
		public static List<Inspection_controller> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM inspection_controller", conn);
        ...
        public static List<Inspection_controller> GetByInspection(int inspection_id)
        {
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT inspection_controller.*,userusers.* FROM inspection_controller INNER JOIN userusers ON inspection_controller.controller_id = userusers.useruserid" +
		...
		public static Inspection_controller GetById(int id)
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM inspection_controller WHERE id = @id", conn);
		...
		public static void Delete(int id, MySqlTransaction tr)
		{
			...
			var cmd = new MySqlCommand("DELETE FROM inspection_controller WHERE id = @id" , conn);
            cmd.Parameters.AddWithValue("@id", id);
            cmd.ExecuteNonQuery();
