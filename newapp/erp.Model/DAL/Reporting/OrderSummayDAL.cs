...
namespace asaq2.Model
{
    ...
    {
        public static List<OrderSummary> GetCreatedInPeriod(DateTime? from, DateTime? to, bool gbie_only = true)
        {
            var result = new List<OrderSummary>();
            using (var conn = new MySqlConnection(Settings.Default.ConnString))
            {
                conn.Open();
                var cmd = new MySqlCommand(string.Format(@"SELECT
                                        ...
                                        INNER JOIN brands ON brands.user_id = cust_products.brand_user_id
                                        ...
                                        order_header.`status` NOT IN ('X', 'Y') AND
                                        customer.distributor > 0 AND order_header.orderdate BETWEEN @from AND @to
                                        {0}
                                        GROUP BY
                                        order_header.orderid,
                                        order_header.orderdate,
                                        order_header.custpo,
                                        customer.customer_code ", gbie_only ? "AND customer.user_country IN ('GB','IE')" : ""), conn);
        ...
        public static List<OrderSummary> GetProducedInPeriod(DateTime? from = null, DateTime? to = null, DateTime? created_until = null, bool gbie_only = true)
        {
            var result = new List<OrderSummary>();
            using (var conn = new MySqlConnection(Settings.Default.ConnString))
            {
                ...
                var cmd = new MySqlCommand(string.Format(@"SELECT
                                        ...
                                        mast_products.category1 <> 13 AND
                                        order_header.`status` NOT IN ('X', 'Y') AND
                                        customer.distributor > 0 AND (order_header.orderdate < @created_until OR @created_until IS NULL)
                                        {0}
                                        GROUP BY
                                        order_header.orderid,
                                        order_header.orderdate,
                                        ...
                                        ", gbie_only ? "AND customer.user_country IN ('GB','IE')" : ""), conn);
                cmd.Parameters.AddWithValue("@from", from != null ? (object)from : DBNull.Value);
                cmd.Parameters.AddWithValue("@to", to != null ? (object)to : DBNull.Value);
