...
namespace asaq2.Model
{
    public class Brand_categories_translateDAL
	{
		...
		public static List<Brand_categories_translate> GetAll(string lang)
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM brand_categories_translate", conn);
		...
		public static Brand_categories_translate GetById(int id, string lang)
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM brand_categories_translate WHERE brand_cat_id = @id AND lang = @lang", conn);
		...
		public static void Delete(int id, string lang)
		{
            ...
            {
                conn.Open();
				var cmd = new MySqlCommand("DELETE FROM brand_categories_translate WHERE brand_cat_id = @id AND lang = @lang" , conn);
                cmd.Parameters.AddWithValue("@id", id);
                cmd.Parameters.AddWithValue("@lang", lang);
                cmd.ExecuteNonQuery();
            }
