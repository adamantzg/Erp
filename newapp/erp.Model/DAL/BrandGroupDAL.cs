...
namespace asaq2.Model
{
    public class BrandGroupDAL
    {
        ...
        public static BrandGroup GetById(int id)
        {
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM brand_grouping WHERE brand_group = @id", conn);
        ...
        { 
            ...
            {
                conn.Open();
                MySqlCommand cmd = new MySqlCommand(GetSelect(@"SELECT brand_grouping.* {0} FROM brand_grouping INNER JOIN web_products ON brand_grouping.brand_group = web_products.brand_group 
