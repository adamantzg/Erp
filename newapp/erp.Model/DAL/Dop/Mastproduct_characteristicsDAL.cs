...
namespace asaq2.Model
{
    public class Mastproduct_characteristicsDAL
	{
		...
		public static List<Mastproduct_characteristics> GetAll()
		{
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand("SELECT * FROM mastproduct_characteristics", conn);
        ...
        public static List<Mastproduct_characteristics> GetByProductId(int id)
        {
            ...
            {
                conn.Open();
                var cmd = new MySqlCommand(@"SELECT mastproduct_characteristics.*, catdop_characteristics.* 
		...
		public static void Create(Mastproduct_characteristics o)
        {
			...
			using (var conn = new MySqlConnection(Properties.Settings.Default.ConnString))
            {
                ...
                MySqlCommand cmd = new MySqlCommand("SELECT MAX(characteristics_id)+1 FROM mastproduct_characteristics", conn);
		...
		public static void Delete(int characteristics_id)
		{
            ...
            {
                conn.Open();
				var cmd = new MySqlCommand("DELETE FROM mastproduct_characteristics WHERE characteristics_id = @id" , conn);
